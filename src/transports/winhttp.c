...
// TODO store needs to survive until http close?
static int handle_client_cert(winhttp_stream *s) {
	HCERTSTORE my_store;
	...
	if (true) {
		...
		do {
			...
			cert->parent.cert_type = GIT_CERT_X509;
			cert->data = cert_context->pbCertEncoded;
...
cleanup:
	git_vector_free_deep(&certs);
	git_vector_foreach(&cert_contexts, i, cert_context) {
		CertFreeCertificateContext(cert_context);
	}
	git_vector_free(&cert_contexts);
...
static int send_request(winhttp_stream *s, size_t len, bool chunked)
{
	...
	while (request_failed && attempts++ < 3) {
		...
		if (client_cert_requested) {
			if ((error = handle_client_cert(s)) != 0) {
				return error; // TODO return or continue?
			}
		}
	}
